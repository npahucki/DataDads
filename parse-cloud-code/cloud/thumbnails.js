...
Parse.Cloud.beforeSave("MilestoneAchievements", function(request, response) {
  ...
  var isImage = achievement.get("attachmentType").indexOf("image/") == 0;
  var needThumbnail = achievement.dirty("attachment");
  if (!isImage || !needThumbnail) {
    console.log("NOT SAVING THUMBNAIL! isImage:" + isImage + " need:" + needThumbnail);
    response.success();
    return;
  }

  makeImageThumbnail(achievement.get("attachment"), 54, 54)
...
Parse.Cloud.beforeSave("Babies", function(request, response) {
  ...
  makeImageThumbnail(baby.get("avatarImage"), 54, 54)
  .then(function(thumbnail) {
