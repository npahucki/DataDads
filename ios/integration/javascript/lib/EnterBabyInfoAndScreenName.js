...
test("expectCanCheckExistingTag", function(target, app) {
  ...
  assertEquals(tagTable.cells()[1].images[0].name,"tagCheckbox");
  tagTable.cells()[1].tap();
  assertEquals(tagTable.cells()[1].images[0].name,"tagCheckbox_checked");
});
...
test("expectCantAddBlankTag", function(target, app) {
  ...
  target.frontMostApp().mainWindow().textFields()["newTagTextField"].tap();
  assertFalse(target.buttons["addNewTagButton"].isEnabled);
  ...
  assertEquals(tagTable.cells()[0].images[0].name(),"tagCheckbox_checked");
...
// Uncheck agree - TODO: assert DOne button disabled. 
